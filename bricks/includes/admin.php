...
class Admin {
	...
	public function __construct() {
		...
		add_filter( 'block_editor_settings_all', [ $this, 'add_gutenberg_frontend_styles' ], 10, 2 );
	...
	public function gutenberg_scripts() {
		...
		if ( $is_block_editor && ! $components_in_block_editor ) {
			/**
			 * Load core block theme styles in Gutenberg so blocks like Quote remain visually distinct.
			 *
			 * Do not use add_theme_support( 'wp-block-styles' ) here, as that also affects frontend output
			 * for existing sites. When components as blocks is enabled, Bricks frontend styles are scoped
			 * and enqueued below, which already gives quotes a visible editor style.
			 *
			 * @since 2.3.6
			 */
		...
		$should_dequeue_iframe_reset_styles = (
			$components_in_block_editor &&
			$is_block_editor &&
			$screen->base === 'post' &&
			Helpers::is_post_type_supported( $post_id ) &&
		...
		if ( $components_in_block_editor ) {
			...
			if ( $global_foundation_css ) {
				wp_add_inline_style(
					...
					\Bricks\Integrations\Block_Editor::scope_css_for_gutenberg( $global_foundation_css )
				);
			}
	...
	public function add_gutenberg_frontend_styles( $editor_settings, $block_editor_context ) {
		...
		$post_id = ! empty( $block_editor_context->post->ID ) ? intval( $block_editor_context->post->ID ) : 0;

		if ( ! $post_id || ! Helpers::is_post_type_supported( $post_id ) || ! Capabilities::current_user_can_use_builder() ) {
			return $editor_settings;
		}
		...
		$style_manager_css_file = Assets::$css_dir . '/style-manager.min.css';

		if ( file_exists( $style_manager_css_file ) ) {
			$style_manager_css = file_get_contents( $style_manager_css_file );

			if ( $style_manager_css ) {
				$editor_settings['styles'][] = [
					'css'            => \Bricks\Integrations\Block_Editor::scope_css_for_gutenberg( $style_manager_css ),
					'__unstableType' => 'theme',
					'isGlobalStyles' => false,
				];
			}
		...
		if ( $theme_style_css ) {
			$editor_settings['styles'][] = [
				'css'            => \Bricks\Integrations\Block_Editor::scope_css_for_gutenberg( $theme_style_css ),
				'__unstableType' => 'theme',
				'isGlobalStyles' => false,
			];
		}
	...
	public function admin_menu() {
		...
		if ( ! current_user_can( 'manage_options' ) ) {
			// Show standalone templates menu for users with template editing permission.
			if ( Builder_Permissions::user_has_permission( 'edit_templates' ) ) {
				...
				add_menu_page(
					...
					self::EDITING_CAP,
					'bricks-templates',
					[ $this, 'admin_screen_templates_redirect' ],
					$menu_icon,
					2
				);
			}
	...
	public function admin_screen_templates_redirect() {
		wp_safe_redirect( admin_url( 'edit.php?post_type=' . BRICKS_DB_TEMPLATE_SLUG ) );
		exit;
	}
