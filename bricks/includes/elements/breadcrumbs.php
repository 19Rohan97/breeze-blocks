...
class Breadcrumbs extends Element {
	...
	public function bricks_breadcrumbs() {
		...
		// Home link
		$home_link  = ! empty( $settings['homeURL'] ) ? esc_url( bricks_render_dynamic_data( $settings['homeURL'], $this->post_id ) ) : esc_url( home_url() );
		$home_label = ! empty( $settings['homeLabel'] ) ? esc_html( bricks_render_dynamic_data( $settings['homeLabel'], $this->post_id ) ) : esc_html__( 'Home', 'bricks' );
