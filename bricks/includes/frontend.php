...
class Frontend {
	...
	public static $faqpage_schema_entities = []; // Aggregated FAQPage mainEntity entries
	...
	public function __construct() {
		...
		add_action( 'wp_footer', [ $this, 'render_faqpage_schema' ], 20 );
	...
	public static function add_faqpage_schema_entities( $entities ) {
		...
		self::$faqpage_schema_entities = array_merge( self::$faqpage_schema_entities, $entities );
	}
	...
	public function render_faqpage_schema() {
		if ( empty( self::$faqpage_schema_entities ) ) {
			return;
		}

		$faqpage_schema = wp_json_encode(
			[
				'@context'   => 'https://schema.org',
				'@type'      => 'FAQPage',
				'mainEntity' => self::$faqpage_schema_entities,
			],
		...
		echo '<script type="application/ld+json">' . $faqpage_schema . '</script>';

		self::$faqpage_schema_entities = [];
	}
	...
	public static function render_element( $element ) {
		...
		$component_instance_id = ! empty( $element['slotInstanceId'] ) ? $element['slotInstanceId'] : ( $element['instanceId'] ?? $element['id'] );
		...
		foreach ( $component_elements as $component_element ) {
			...
			if ( $component_element['name'] === 'slot' ) {
				...
				// Other elements: Use local slot payload ID when expanding a slotted component instance.
				// (#86ca4tpmq; @since 2.3.8)
				$component_element['instanceId'] = $component_instance_id;
			}
		...
		/**
		 * STEP: Add slot children and descendants to Frontend::$elements if component instance has slots
		 *
		 * Nested slot content (e.g. Slot > Div > Text) must exist by its original local ID during
		 * partial renders, otherwise only the direct slot child can render after AJAX pagination.
		 * (#86c7p02wq; @since 2.3.8)
		 *
		 * @since 2.2
		 */
		if ( ! empty( $element['slotChildren'] ) ) {
			$slot_source_elements = [];
			$slot_source_area     = in_array( self::$area, [ 'header', 'content', 'footer' ], true ) ? self::$area : 'content';
			$slot_source_id       = Database::$active_templates[ $slot_source_area ] ?? 0;
			$slot_source_id       = is_array( $slot_source_id ) ? 0 : intval( $slot_source_id );

			if ( ! $slot_source_id ) {
				$slot_source_id = Database::$page_data['preview_or_post_id'] ?? 0;
			}

			/**
			 * Read slot descendants from the active render area first.
			 *
			 * Header/footer/content templates can own component instances with slotChildren.
			 * Falling back to content-only data would miss those stored descendants in partial renders.
			 * (#86c7p02wq; @since 2.3.8)
			 */
			$original_elements = $slot_source_id ? Database::get_data( $slot_source_id, $slot_source_area ) : [];

			if ( is_array( $original_elements ) ) {
				$slot_source_elements = $original_elements;
			}

			// Current render context wins over stored page data so AJAX/builder overrides stay intact.
			$slot_source_elements = array_merge( $slot_source_elements, self::$elements );

			foreach ( Helpers::get_component_slot_child_elements( $element, $slot_source_elements ) as $slot_child_element ) {
				self::$elements[ $slot_child_element['id'] ] = $slot_child_element;
			}
		}
	...
	private static function maybe_store_live_search_wrapper_selector( $element ) {
		if ( ! Helpers::enabled_query_filters() || empty( $element['settings']['hasLoop'] ) ) {
			return;
		}

		$query_settings = $element['settings']['query'] ?? [];
		$query_settings = is_array( $query_settings ) ? $query_settings : [];

		if ( ! empty( $query_settings['id'] ) ) {
			// Resolve global query settings before checking live search flags (#86c95znhp; @since 2.3.6).
			$query_settings = Helpers::maybe_get_global_query_settings( $query_settings );
		}

		if ( empty( $query_settings['is_live_search'] ) || empty( $query_settings['is_live_search_wrapper_selector'] ) ) {
			return;
		}

		$live_search_query_id = $element['id'] ?? '';

		// Match Query::__construct() IDs for query loops inside component instances.
		if ( ! empty( $element['instanceId'] ) && ! empty( $element['parentComponent'] ) && strpos( $live_search_query_id, '-' ) === false ) {
			$live_search_query_id .= '-' . $element['instanceId'];
		}

		if ( $live_search_query_id ) {
			self::$live_search_wrapper_selectors[ $live_search_query_id ] = $query_settings['is_live_search_wrapper_selector'];
		}
	}
	...
	public static function render_data( $elements = [], $area = 'content' ) {
		 ...
		 *
		 * Component instance properties can provide query settings to the component root/children,
		 * so expand component data before scanning for live search loops.
		 *
		 * @since 1.9.6
		 */
		if ( Helpers::enabled_query_filters() ) {
			// Consider component instances with live search query settings as well (#86c2zdpr1; @since 2.3.8).
			$live_search_elements = Database::get_component_data( $elements );

			foreach ( $live_search_elements as $element ) {
				self::maybe_store_live_search_wrapper_selector( $element );
			}
		}
