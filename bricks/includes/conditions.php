...
class Conditions {
	...
	public static function get_current_url() {
		...
		// AJAX requests do not include the original browser URL/query string. (#86c21d4m0; @since 2.3.8)
		if ( ! empty( Api::$request_data['baseUrl'] ) ) {
			$url_params = [];

			if ( ! empty( Api::$request_data['urlParams'] ) && is_array( Api::$request_data['urlParams'] ) ) {
				$url_params = Api::$request_data['urlParams'];
			}

			$value = add_query_arg( $url_params, Api::$request_data['baseUrl'] );
		}
