...
/**
 * Render Woo notice if not added via Bricks {do_action:woocommerce_before_cart} in cart template
 *
 * @since 1.7
 */
...
if ( ! Bricks\Woocommerce::use_bricks_woo_notice_element() ) {
	...
	$has_woo_notice_do_action = is_array( $cart_template_elements ) && strpos( wp_json_encode( $cart_template_elements ), '{do_action:woocommerce_before_cart}' ) !== false;
	if ( ! $has_woo_notice_do_action ) {
		echo '<div class="brxe-container before-cart">';
			...
			foreach ( WC()->cart->get_cart() as $cart_item_key => $cart_item ) {
				...
				if ( $_product instanceof WC_Product && $_product->exists() && $cart_item['quantity'] > 0 && $visible ) {
					...
					<tr class="woocommerce-cart-form__cart-item <?php echo esc_attr( apply_filters( 'woocommerce_cart_item_class', 'cart_item', $cart_item, $cart_item_key ) ); ?>">
						...
						<th scope="row" role="rowheader" class="product-name" data-title="<?php esc_attr_e( 'Product', 'woocommerce' ); ?>">
						<?php
						if ( ! $product_permalink ) {
...
<div class="cart-collaterals">
	...
	<?php
	// @since 1.7 - Should place {do_action:woocommerce_after_cart} in the user custom Cart template
